...
- (void)setupTutorialsTableDataSource
{
  ...
  self.tutorialsTableDataSource.predicate = [NSPredicate predicateWithFormat:@"flaggedAsInappropriate == 0 AND reportedByUser == 0 AND createdBy = %@", self.user];
...
{
  self.tutorialsTableDataSource = [self createTutorialsTableDataSourceNoPredicate];
  self.tutorialsTableDataSource.predicate = [NSPredicate predicateWithFormat:@"flaggedAsInappropriate == 0 AND reportedByUser == 0 AND %@ IN likedBy", self.user]; // TODO: would be much faster other way round - just displaying user.likes...
}
